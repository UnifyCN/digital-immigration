...
function LanguageScoresList({ assessment }: { assessment: AssessmentData }) {
  ...
  const hasAnyScore = Boolean(
    scores.listening.trim() || scores.reading.trim() || scores.writing.trim() || scores.speaking.trim(),
  )
...
function CanadianWorkSnapshot({ assessment }: { assessment: AssessmentData }) {
  const currentRoleInCanada = isCanadaCountry(assessment.countryOfWork) ? 1 : 0
  const quickAddRolesInCanada = (assessment.jobs ?? []).filter(
    (job) => isCanadaCountry(job.country),
  ).length
  const canadianRoleCount = currentRoleInCanada + quickAddRolesInCanada

  if (canadianRoleCount === 0) {
    return null
  }
  ...
  return (
    <div className="py-2">
      <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
        Canadian work snapshot
      </p>
      <KeyValueRow label="Canadian work entries" value={`${canadianRoleCount} ${canadianRoleCount === 1 ? "role" : "roles"}`} />
      <KeyValueRow label="Canadian work authorization" value={formatAnswer(assessment.canadianWorkAuthorizedAll)} />
    </div>
  )
}
            ...
            <AccordionItem key={section.id} value={section.id}>
              ...
              <AccordionContent>
                ...
                <div className="mt-2">
                  ...
                  {section.id === "work" && <CanadianWorkSnapshot assessment={assessment} />}
