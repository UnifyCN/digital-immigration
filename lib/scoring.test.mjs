...
function baseProfile() {
  const profile = structuredClone(defaultAssessmentData)
...
test("Express Entry card appears for eligible profile", () => {
  ...
  assert.equal(eeCard?.statusTag, PATHWAY_STATUS.ELIGIBLE)
})
...
test("Express Entry card appears as Needs info when unresolved", () => {
  ...
  assert.equal(eeCard?.statusTag, PATHWAY_STATUS.NEEDS_INFO)
})
...
test("Express Entry card hides when definitively ineligible", () => {
  ...
  assert.ok(
    result.expressEntryEligibility.programs.fsw.reasonDetails.some(
      (reason) => reason.kind === "fail" || reason.kind === "missing",
    ),
  )
