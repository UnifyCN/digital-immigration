...
test("example: PR-intent profile with active job offer builds expected core signals", () => {
  ...
  assert.equal(signals.languageReady, "not_ready")
...
test("example: unknown handling and meta unknownRate counts only KEY_FIELDS", () => {
  ...
  assert.equal(meta.unknownRate, 13 / KEY_FIELDS.length)
})
...
test("example: outside-canada profiles exclude inside-status fields from unknown-rate denominator", () => {
  const { meta } = buildPNPSignals({
    primaryGoal: "pr",
    openToPNP: "yes",
    currentLocation: "outside-canada",
  })

  assert.equal(meta.keyFieldsTotal, KEY_FIELDS.length - 2)
})
