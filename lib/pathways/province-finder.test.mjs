...
test("supplemental validation requires NOC/TEER and EE basics", () => {
  const errors = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
    ...
    context: {
      hasJobOffer: false,
      currentlyWorkingInCanada: false,
      outsideCanada: true,
    },
  })

  assert.ok(errors.includes("Please answer: Do you know your NOC code?"))
  assert.ok(errors.includes("Please answer: Do you currently have an active Express Entry profile?"))
})

test("supplemental validation enforces conditional CRS and employer checks", () => {
  const errors = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
  ...
  assert.ok(errors.includes("Please enter your CRS score."))
  assert.ok(
    errors.includes(
...
test("supplemental progress helper counts NOC/EE branches and employer eligibility deterministically", () => {
  const branchYes = computeSupplementalCompletedCount({
    answers: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: 510,
      ...
      ee_profile_active: "yes",
      ee_crs_known: "yes",
      employer_operation_years_in_province: "3_5_years",
    },
    showEmployerParticipationDetails: true,
  })
  const branchNo = computeSupplementalCompletedCount({
    answers: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
      noc_known: "no_not_sure",
      teer_level_guess: "teer_2_technical_skilled_trades",
      ee_profile_active: "yes",
      ee_crs_known: "no",
      ee_crs_range: "450_499",
    },
    showEmployerParticipationDetails: false,
  })

  assert.equal(branchYes, 5)
  assert.equal(branchNo, 4)
})

test("supplemental validation rejects malformed NOC code values", () => {
  const badAlpha = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
  ...
  const badShort = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
  ...
  assert.ok(badAlpha.includes("NOC code must be 4-5 digits."))
  assert.ok(badShort.includes("NOC code must be 4-5 digits."))
})

test("supplemental validation enforces CRS score integer boundaries and CRS range when unknown", () => {
  const validZero = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: 0,
      noc_known: "no_not_sure",
      teer_level_guess: "not_sure",
      ee_profile_active: "yes",
      ee_crs_known: "yes",
    },
    context: {
  ...
  const validUpper = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: 1200,
  ...
  const invalidNegative = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: -1,
  ...
  const invalidOverMax = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: 1201,
  ...
  const missingRange = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
  ...
  assert.ok(!validZero.includes("Please enter your CRS score."))
  assert.ok(!validZero.includes("CRS score must be a whole number between 0 and 1200."))
  assert.ok(!validUpper.includes("CRS score must be a whole number between 0 and 1200."))
  assert.ok(invalidNegative.includes("CRS score must be a whole number between 0 and 1200."))
  assert.ok(invalidOverMax.includes("CRS score must be a whole number between 0 and 1200."))
  assert.ok(missingRange.includes("Please select your estimated CRS range."))
})

test("supplemental validation allows optional postal code to be empty and rejects invalid values", () => {
  const withEmptyPostal = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
  ...
  assert.ok(!withEmptyPostal.includes("Please enter a valid Canadian postal code (e.g., A1A 1A1)."))

  const withInvalidPostal = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
      ...
      teer_level_guess: "not_sure",
      ee_profile_active: "no",
      employer_operation_years_in_province: "1_2_years",
      job_location_postal_code: "12345",
    },
    context: {
      hasJobOffer: true,
      currentlyWorkingInCanada: false,
  ...
  assert.ok(withInvalidPostal.includes("Please enter a valid Canadian postal code (e.g., A1A 1A1)."))

  const withValidPostal = validateProvinceFinderSupplementalAnswers({
    draft: {
      ...buildBaseAnswers(),
      hourlyWage: 20,
      ee_crs_score: null,
      ...
      ee_profile_active: "no",
      employer_operation_years_in_province: "1_2_years",
      job_location_postal_code: "K1A 0B1",
    },
    context: {
      hasJobOffer: true,
      currentlyWorkingInCanada: false,
  ...
  assert.ok(!withValidPostal.includes("Please enter a valid Canadian postal code (e.g., A1A 1A1)."))
})
