...
test("CEC eligible profile returns eligible", () => {
  ...
  assert.ok(result.reasonDetails.some((reason) => reason.code === "cec_passed"))
})
...
test("expired language test returns non-eligible outcome", () => {
  ...
  const result = checkCEC(profile, AS_OF_DATE)
  assert.equal(result.status, "ineligible")
  assert.ok(result.reasonDetails.some((reason) => reason.code === "language_test_expired"))
})
...
test("expired ECA produces FSW needs_more_info", () => {
  ...
  assert.ok(result.reasonDetails.some((reason) => reason.code === "eca_missing_or_expired"))
})
...
test("CEC excludes unauthorized work", () => {
  const profile = baseProfile()
  applyValidPrimaryLanguage(profile)
  applyCanadianWorkRole(profile, {
...
test("CEC excludes self-employed work", () => {
  const profile = baseProfile()
  applyValidPrimaryLanguage(profile)
  applyCanadianWorkRole(profile, {
    wasAuthorizedInCanada: "yes",
...
test("FST eligible case passes", () => {
  ...
  assert.equal(result.status, "eligible")
  assert.ok(result.reasonDetails.some((reason) => reason.code === "fst_passed"))
})
...
test("aggregate result returns needs_more_info and missing fields", () => {
  ...
  assert.ok(result.programs.cec.reasonDetails.length > 0)
})
